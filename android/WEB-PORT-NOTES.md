...
The summary under a list reads “X tracks/albums/artists · Y scrobbles”; a scrobble list shows only its scrobble count. Comparison and Reset filters share a quiet outlined style. Filters is wider, taller and filled because it is the primary action in the row. Comparison may gain a subtle container color when active. The active-filter strip consists only of clickable chips plus its expand/collapse control; do not repeat the same values in a separate settings-style text row.
